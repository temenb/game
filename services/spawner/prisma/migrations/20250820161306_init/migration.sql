/*
  ...
  - You are about to drop the column `isDestroyed` on the `Spawn` table. All the data in the column will be lost.
  - Added the required column `spawn_grade` to the `Spawn` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "public"."Spawn" DROP COLUMN "isDestroyed",
...
-- CreateIndex
CREATE INDEX "Spawn_galaxy_id_idx" ON "public"."Spawn"("galaxy_id");

-- CreateIndex
CREATE INDEX "Spawn_owner_id_idx" ON "public"."Spawn"("owner_id");

-- CreateIndex
CREATE INDEX "Spawn_axisX_axisY_idx" ON "public"."Spawn"("axisX", "axisY");
